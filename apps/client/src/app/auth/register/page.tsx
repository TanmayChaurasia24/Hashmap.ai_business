...
const SignupForm = () => {
  ...
  const handleSignup = async(e: any) => {
    e.preventDefault();
    console.log("inside handleSignup", companyData);
    ...
    if(!email || !password || !name || !website || !description || !location || !industry || !size) {
      toast.error("please enter every detail properly!")
      return;
    }
    ...
    try {
      ...
      const response = await axios.post("http://localhost:8080/create/company", companyData, {
        headers: {
      ...
      <div className="w-1/2 bg-gradient-to-r from-slate-900 to-black p-8 overflow-y-auto">
        ...
        >
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-bold text-neutral-200">Register Your Company</h2>
            <p className="text-gray-500 mt-2">Join thousands of companies using our platform</p>
          </div>

          <form className="space-y-6">
            <motion.div variants={fadeIn} className="space-y-4">
              <div className="relative">
                <Building2 className="absolute top-[38px] left-3 h-5 w-5 text-black" />
                <label className="block text-sm font-medium text-neutral-200">Company Name</label>
                <input
                  type="text"
                  name="name"
              ...
              <div className="relative">
                <Globe className="absolute top-[38px] left-3 h-5 w-5 text-black" />
                <label className="block text-sm font-medium text-neutral-200">Website</label>
                <input
                  type="url"
                  name="website"
                  value={companyData.website}
                  onChange={handleChange}
                  className="mt-1 block w-full pl-10 pr-3 py-2 borderc text-black border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                  placeholder="https://"
                />
              </div>
              <div className="relative">
                <label className="block text-sm font-medium text-neutral-200">Location</label>
                <input
                  type="text"
                  name="location"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  className="mt-1 block w-full px-3 py-2 border text-black border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                  rows={3}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="relative">
                  <Briefcase className="absolute top-[38px] left-3 h-5 w-5 text-black" />
                  <label className="block text-sm font-medium text-neutral-200">Industry</label>
                  <select
                    name="industry"
                ...
                <div className="relative">
                  <Users className="absolute top-[38px] left-3 h-5 w-5 text-black" />
                  <label className="block text-sm font-medium text-neutral-200">Company Size</label>
                  <select
                    name="size"
                    value={companyData.size}
                    onChange={handleChange}
                    className="mt-1 block w-full pl-10 pr-3 py-2 text-black border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                  >
